...
//Rare exceptions to indicate a bug in the _intramuralsports code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_intramuralsports = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_intramuralsports]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.intramuralsports.module"
